...
//find the position in the partsKeys list
	...
	var wd = parseFloat(ddiv.style.width.slice(0,-2)) + offset; //to take off "px"

	keys = Object.keys(params.gtoggle)
	pdiv = document.getElementById(keys[i]+'Div');

	pdiv = d3.select('#'+keys[i]+'Div').selectAll('div')
	console.log(pdiv.node())
	if (params.gtoggle[pID]){
		pdiv.node().forEach(function(d){
			d.style('margin-left',wd + "px");
		})
	} else {
